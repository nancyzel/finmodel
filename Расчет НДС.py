...
df=pd.read_excel('test.xlsx', sheet_name='Расчет НДС')
...
def update_row_data(cell_changed,data):
    ...
    for el in sp1:
        ...
        data[11][el]=sum([data[i][el] for i in range(3,11)])
        data[12][el]=data[2][el]+data[11][el]
        data[13][el]=data[12][el]
        if el!='0':
            data[16][el]=data[17][sp1[sp1.index(el)-1]]
        data[17][el]=data[12][el]+data[14][el]+data[15][el]+data[16][el]+data[2][el]
        data[18][el]=data[3][el]+data[4][el]+data[5][el]+data[6][el]
        data[19][el]=data[0][el]+data[1][el]+data[7][el]+data[8][el]+data[9][el]+data[10][el]
    for i in range(3):
        data[i]['summa']=sum([data[i][x] for x in sp2])
    for i in range(3,13):
        data[i]['summa']=sum([data[i][x] for x in sp1])
    data[13]['summa']=data[13]['14']
    for i in range(14,16):
        data[i]['summa']=sum([data[i][x] for x in sp1])
    data[16]['summa']=data[16]['0']
    data[17]['summa']=data[17]['14']
...
def update_portfolio_stats(n, data):
    ...
    with pd.ExcelWriter('test.xlsx', mode="a", engine="openpyxl", if_sheet_exists='replace') as writer:
        dff.to_excel(writer, sheet_name="Расчет НДС", index=False)
